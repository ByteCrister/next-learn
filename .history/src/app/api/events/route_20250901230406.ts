...
export async function POST(req: NextRequest) {
    try {
        ...
        const saved = await Event.findById(newEvent._id).lean();
        if (!saved) {
            return NextResponse.json({ message: "Event not found after creation" }, { status: 500 });
        }

        const now = Date.now();
        const start = new Date(saved.start as Date).getTime();
        const end = start + ((saved.durationMinutes as number) * 60 * 1000);
        const allDone = (saved.tasks as any[]).every((t: any) => t.isComplete);
