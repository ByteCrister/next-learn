...
export async function POST(req: NextRequest) {
    try {
        ...
        const { title, description, start, durationMinutes, allDay, tasks } = await req.json();

        // Validate that tasks array exists and has at least one task
        if (!tasks || !Array.isArray(tasks) || tasks.length === 0) {
            return NextResponse.json(
                { error: "An event must have at least one task" },
                { status: 400 }
            );
        }
