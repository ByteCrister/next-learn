...
export async function GET(req: NextRequest) {
    try {
        ...
        // Compute currentStatus for each event since lean() doesn't include virtuals
        const eventsWithStatus = events.map((evt) => {
            const now = Date.now();
            const start = new Date(evt.start).getTime();
            const end = start + (evt.durationMinutes * 60 * 1000);
            const allDone = evt.tasks.every((t: any) => t.isComplete);

            let currentStatus: "upcoming" | "inProgress" | "expired" | "completed";
            if (allDone) {
            ...
            return {
                ...evt,
                currentStatus,
                end: new Date(end), // also compute end
            };
        });
...
export async function POST(req: NextRequest) {
    try {
        ...
        // Re‐fetch and compute currentStatus
        const saved = await Event.findById(newEvent._id).lean() as any;
        if (!saved) {
            return NextResponse.json({ message: "Event not found after creation" }, { status: 500 });
        }

        const now = Date.now();
        const start = new Date(saved.start).getTime();
        const end = start + (saved.durationMinutes * 60 * 1000);
        const allDone = saved.tasks.every((t: any) => t.isComplete);
        ...
        return NextResponse.json(savedWithStatus, { status: 201 });
    } catch (error) {
        return handleError(error);
    }
}
...
export async function PUT(req: NextRequest) {
    try {
        ...
        const returned = await Event.findById(id).lean() as any;
        if (!returned) {
            return NextResponse.json({ message: "Event not found after update" }, { status: 500 });
        }

        const now = Date.now();
        const start = new Date(returned.start).getTime();
        const end = start + (returned.durationMinutes * 60 * 1000);
        const allDone = returned.tasks.every((t: any) => t.isComplete);
        ...
        const returnedWithStatus = {
            ...returned,
            currentStatus,
            end: new Date(end),
        };

        return NextResponse.json(returnedWithStatus);
    } catch (error) {
        return handleError(error);
    }
}
