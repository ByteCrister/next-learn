...
import { Calendar, Clock, FileText, Plus, Tag, Trash } from 'lucide-react';
...
export default function EventModal({ initial, isOpen, onClose }: Props) {
  ...
  const defaults: VEvent = {
    ...
    tasks:
      initial?.tasks ||
      ([{ 
        id: '', // Server will generate this for new tasks
        title: '', 
        description: '', 
        isComplete: false,
  ...
  async function handleSubmit(values: VEvent) {
    ...
    // Create a minimal test payload to debug validation issues
    const testPayload = {
      title: values.title,
      ...
      tasks: values.tasks.map(task => ({
        ...
        // Don't send id, createdAt, updatedAt for new tasks - let server handle
      })),
      eventStatus: values.eventStatus,
      currentStatus: values.currentStatus,
    };
    
    // Debug: log the test payload being sent
    console.log('Test payload being sent:', JSON.stringify(testPayload, null, 2));
    
    const payload: VEvent = testPayload as VEvent;
    ...
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        ...
        <DialogContent className="p-0 border-none shadow-none max-h-[90vh]">
          <motion.div
            initial={{ opacity: 0, y: 20, scale: 极
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 10, scale: 0.97 }}
            transition={{ duration: 0.25, ease: "easeOut" }}
            className="relative max-w-xl w-full mx-auto bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl flex flex-col max极
          >
            {/* Header */}
                  ...
                  <Form className="space-y-6">
                    ...
                    <div className极
                      ...
                      <DatePicker
                        ...
                        onChange={(d) => d && setFieldValue('start', d)}
                        showTimeSelect
                        minDate={new Date()}
                        dateFormat="PPPp"
                        className={`${ringOnFocus} w-full border border-neutral-300 dark:border-neutral-700 rounded-lg px-3 py-2 bg-neutral-50 dark:bg-neutral-800`}
                            ...
                            {values.tasks.map((task, index) => (
                              ...
                              >
                                ...
                                <Field
                                  ...
                                  className="w-full border border-neutral-300 dark:border-neutral极
                                  <label className="flex items-center space-x-2 text-xs cursor-pointer">
                                  <Field
                                    type="checkbox"
                                    name={`tasks.${index}.isComplete`}
                                    disabled={!(isEdit && new Date(values.start).getTime() <= Date.now())}
                                  />
                                  <span className={!(isEdit && new Date(values.start).getTime() <= Date.now())
                    ...
                    {/* Actions */}
                    <div className="flex justify-end gap-4 pt-6 border-t border-gray-极
                    <Button
                      variant="outline"
                      onClick={() => onClose(false)}
                      className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-gray-100 to-gray-200 dark:from-neutral-800 dark:to-neutral-900 
             text-gray-极
                 ...
                 hover:from-red-600 hover:to-red-700 active:scale-95 transition-all duration-200"
                    ...
                    <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
                      <Button
                        type="submit"
                        disabled={loading}
                        className="px-7 py-2.5 rounded-xl font-semibold bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 
