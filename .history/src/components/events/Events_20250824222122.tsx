              ...
              : paginatedEvents.length === 0
                ...
                : paginatedEvents.map((evt) => {
                  ...
                  return (
                    ...
                    >
                      <Card className={`rounded-3xl overflow-hidden border border-indigo-100 shadow-md transition-all duration-300 hover:shadow-indigo-hover ${getCardStatusColor(evt.eventStatus)}`}>

                        {/* Inner Background Layer */}
                        <div className="bg-white/95 px-0 py-0">
                          <CardHeader className="px-4 sm:px-6 pt-4 sm:pt-6 pb-2 sm:pb-3 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                            <Badge
                              className={`rounded-full px-2 sm:px-3 py-0.5 sm:py-1 text-xs font-semibold tracking-wide shadow-sm ${getStatusColor(evt.eventStatus)} uppercase w-fit`}
                            >
                              {evt.eventStatus}
                            </Badge>
                            {evt.tasks?.length > 0 && (
                              <span className="text-xs font-medium bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full shadow-inner w-fit">
                                {evt.tasks.length} Tasks
                              </span>
                            )}
                          </CardHeader>

                          <CardContent className="px-4 sm:px-6 pb-4 sm:pb-6 space-y-3 sm:space-y-5">
                            <CardTitle className="text-base sm:text-lg font-bold text-indigo-900 line-clamp-2">
                              {evt.title}
                            </CardTitle>
                            {/* Description */}
                            <p className="line-clamp-3 text-gray-700 leading-relaxed text-xs sm:text-sm">{evt.description || 'No description provided.'}</p>

                            <hr className="border-gray-100" />

                            {/* Dates */}
                            <div className="text-indigo-900 text-xs sm:text-sm space-y-1">
                              <p className="flex flex-col sm:flex-row sm:items-center gap-1">
                                <span className="font-medium text-gray-500 text-xs">Start:</span>
                                <span className="font-semibold break-words">{toDate(evt.start)?.toLocaleString() || '—'}</span>
                              </p>
                              <p className="flex flex-col sm:flex-row sm:items-center gap-1">
                                <span className="font-medium text-gray-500 text-xs">End:</span>
                            ...
                            {evt.tasks?.length > 0 && (
                              <div className="space-y-2">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-xs font-medium text-gray-600 gap-1">
                                  <div className="flex items-center gap-2">
                                    <CheckCircle2 size={12} className="text-green-500" />
                                    <span className="text-xs">{doneCount} / {totalCount} tasks complete</span>
                                  </div>
