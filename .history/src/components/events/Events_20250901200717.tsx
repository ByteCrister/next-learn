                      ...
                      <Card className={`px-0 py-0 rounded-3xl overflow-hidden border border-indigo-100 shadow-md transition-all duration-300 hover:shadow-indigo-hover cursor-pointer ${getCardStatusColor(evt.eventStatus)}`}>
                        ...
                        <div className={`px-0 py-0 rounded-3xl overflow-hidden border border-indigo-100 shadow-md transition-all duration-300 hover:shadow-indigo-hover ${getCardStatusColor(evt.eventStatus)}`}>
                          ...
                          <CardContent className="px-6 pb-6 space-y-5">
                            ...
                            {/* Description */}
                            <p className="line-clamp-3 text-gray-200 leading-relaxed text-sm">
                              {evt.description
                            ...
                            {/* Hover Buttons */}
                            <div className="absolute top-27 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                              <Button
                                size="sm"
                                variant="default"
                                onClick={(e) => {
                                ...
                                className="flex items-center gap-1 bg-indigo-500 hover:bg-indigo-600 text-white cursor-pointer"
                              >
                                <Eye size={16} />
                                View
                              </Button>
                              <Button
                                ...
                                className="flex items-center gap-1 bg-orange-500 hover:bg-orange-600 text-white cursor-pointer"
                              >
                                <Edit size={16} />
                                Edit
                              </Button>
                            ...
                            <div className="text-gray-100 text-sm space-y-1">
                              <p>
                                ...
                                <span className="font-medium text-gray-100">Start:</span>{' '}
                                <span className="font-semibold text-gray-100">{toDate(evt.start)?.toLocaleString() || '—'}</span>
                              </p>
                              <p>
                                <Clock size={14} className="inline mr-1" />
                                <span className="font-medium text-gray-100">End:</span>{' '}
                              ...
                              {evt.allDay && <p className="text-gray-100 italic text-xs font-semibold">All Day Event</p>}
                              {!evt.allDay && <p className="text-gray-100 italic text-xs font-semibold">Session Event</p>}
                            </div>

                            {/* Task Progress */}
                            {evt.tasks?.length > 0 && (
                              <div className="space-y-2">
                                <div className="flex items-center justify-between text-xs font-medium text-gray-100">
                                  <div className="flex items-center gap-2">
                                    <CheckCircle2 size={14} className="text-green-500" />
                                    {doneCount} / {totalCount} tasks complete
                                  </div>
                                  <span className="text-xs text-gray-100">{Math.round((doneCount / totalCount) * 100)}%</span>
                                </div>
