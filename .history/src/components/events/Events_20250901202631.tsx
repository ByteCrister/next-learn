...
const cardStatusColors: Record<VEvent['eventStatus'], string> = {
  upcoming: "bg-gradient-to-br from-purple-500 to-indigo-600 text-white",
  inProgress: "bg-gradient-to-br from-orange-400 to-yellow-400 text-black",
  expired: "bg-gradient-to-br from-slate-500 to-gray-600 text-white",
  completed: "bg-gradient-to-br from-teal-500 to-green-600 text-white"
};
                      ...
                      <Card className={`px-0 py-0 rounded-3xl overflow-hidden border border-indigo-100 shadow-md transition-all duration-300 hover:shadow-indigo-hover cursor-pointer ${getCardStatusColor(evt.eventStatus)}`}>
                        ...
                        <div className={`px-0 py-0 rounded-3xl overflow-hidden border border-indigo-100 shadow-md transition-all duration-300 hover:shadow-indigo-hover ${getCardStatusColor(evt.eventStatus)}`}>
                          <CardHeader className="px-6 pt-6 pb-3 flex justify-between items-start">
                            <Badge
                              className={`rounded-full px-4 py-1 text-xs font-semibold tracking-wide shadow-sm flex items-center gap-1 uppercase ${getStatusColor(evt.eventStatus)}`}
                            >
                              {evt.eventStatus === 'completed' && <CheckCircle2 size={14} />}
                              {evt.eventStatus}
                            </Badge>
                            {evt.tasks?.length > 0 && (
                              <Badge className="rounded-full px-3 py-1 text-xs font-medium bg-indigo-100 text-indigo-700 shadow-inner">
