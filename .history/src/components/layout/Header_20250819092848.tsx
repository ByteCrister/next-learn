            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navLinks.map((link) => (
                <motion.div key={link.label} variants={itemVariants}>
                  <Link
                    href={link.href}
                    className="text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white block px-3 py-2 rounded-md text-base font-medium transition-all duration-300"
                  >
                    {link.label}
                  </Link>
                </motion.div>
              ...
              <motion.div variants={itemVariants} className="w-full mt-2">
                <Link href={`/user-signup?callbackUrl=${encodeURIComponent(pathname)}`} className="w-full">
