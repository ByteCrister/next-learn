...
export default function Page({
    ...
    const unwrappedParams = React.use(params) as { subjectId: string };

    useEffect(() => {
        fetchSubjectById(unwrappedParams.subjectId);
    }, [unwrappedParams.subjectId, fetchSubjectById]);
    ...
    useEffect(() => {
        if (selectedSubject) {
            ...
            import('@/store/useBreadcrumbStore').then(({ useBreadcrumbStore }) => {
                const setBreadcrumbs = useBreadcrumbStore.getState().setBreadcrumbs;
                setBreadcrumbs([
                    { label: 'Home', href: '/' },
                    { label: 'Subjects', href: '/subjects' },
                    { label: subjectTitle, href: `/subjects/${unwrappedParams.subjectId}` },
                    { label: 'ChaptersList', href: `/subjects/${unwrappedParams.subjectId}/chapters` }
                ]);
            });
        }
    }, [selectedSubject, unwrappedParams.subjectId]);
