...
export default function Page({
    ...
    const unwrappedParams = React.use(params) as { subjectId: string; chapterId: string };
    ...
    // Dynamic SEO
    useEffect(() => {
        const chapter = chapters.find((ch) => ch._id === unwrappedParams.chapterId);
        if (chapter) {
    ...
    }, [chapters, unwrappedParams.chapterId]);
    ...
    return (
        <div className="min-h-screen p-8 bg-gradient-to-br from-indigo-50 to-white">
            <ChapterEditor roadmapId={roadmapId} chapterId={unwrappedParams.chapterId} mode={mode} />
        </div>
    );
}
