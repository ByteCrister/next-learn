  ...
  try {
    ...
    if ("name" in raw) {
      if (typeof raw.name !== "string" || !raw.name.trim())
        return NextResponse.json(
          { status: 400, message: "Invalid name" },
          { status: 400 }
        );
    ...
    if ("year" in raw) {
      ...
      if (raw.year != null && raw.year !== "" && parsedYear == null)
        return NextResponse.json(
          { status: 400, message: "Invalid year" },
          { status: 400 }
        );
    ...
    if ("updatedBy" in raw) {
      ...
      if (raw.updatedBy != null && !oid)
        return NextResponse.json(
          { status: 400, message: "Invalid updatedBy id" },
          { status: 400 }
        );
    ...
    if ("semesters" in raw) {
      const { value: semestersSanitized, errors: semErrors } =
        sanitizeSemesters(raw.semesters);
      if (semErrors.length > 0) {
        return NextResponse.json(
          { status: 400, message: "Invalid semesters", details: semErrors },
          { status: 400 }
        );
      }
    ...
    return NextResponse.json(
      { status: 500, message: "Failed to update batch", details: message },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req: NextRequest, context: unknown) {
  try {
    ...
    const raw = await req.json().catch(() => ({}));
    const deletedByRaw =
      raw && typeof raw === "object" && "deletedBy" in raw
        ? (raw as Record<string, unknown>).deletedBy
        : undefined;
    const deletedBy = toObjectIdIfValid(
      typeof deletedByRaw === "string" ? deletedByRaw : undefined
    );
