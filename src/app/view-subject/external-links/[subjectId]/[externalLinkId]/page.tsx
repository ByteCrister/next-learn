...
import { Calendar, Hash, FileText, BookOpen, Star, ExternalLink } from "lucide-react";
...
export default function ViewSharedExternalLinkPage() {
  ...
  const { subjectId: encodedSubjectId, externalLinkId: encodedExternalLinkId } = useParams();
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const externalLinkId = decodeId(decodeURIComponent(encodedExternalLinkId as string));

      const res = await getViewExternalLink(encodedSubjectId as string, encodedExternalLinkId as string);
      if ("message" in res) {
    ...
    setBreadcrumbs([
      { label: 'Home', href: '/' },
      { label: 'Shared External Link', href: `/view-subject/external-links/${encodedSubjectId}/${encodedExternalLinkId}` },
    ]);
  }, [setBreadcrumbs, encodedSubjectId, encodedExternalLinkId]);
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-3">
                          <iframe
                            src={data.externalLink.url}
                            title={`Embedded view of ${data.externalLink.title}`}
