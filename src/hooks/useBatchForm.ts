...
import { COURSE_TYPE, CreateBatchPayload, EXAM_TYPE, EXAM_TYPE_CONDITION, RESULT_COMPONENT_NAME, UpdateBatchPayload } from "@/types/types.batch";
...
export type CreateProps = CommonProps & { mode?: "create"; onSubmit: (payload: CreateBatchPayload) => Promise<void> };
export type UpdateProps = CommonProps & { mode: "update"; onSubmit: (payload: UpdateBatchPayload) => Promise<void>; id: string };
...
type TeacherInput = { _uid: string; name: string; designation?: string; notes?: string };
type ResultComponentDefInput = { _uid: string; name: RESULT_COMPONENT_NAME; maxMarks: number };
type ExamDefinitionInput = { _uid: string; examType: EXAM_TYPE; condition: EXAM_TYPE_CONDITION; totalMarks?: number | null; components: ResultComponentDefInput[] };
type CoursePartInput = { _uid: string; courseType: COURSE_TYPE; credits: number; teachers: TeacherInput[]; examDefinitions: ExamDefinitionInput[]; notes?: string };
type CourseAssignmentInput = { _uid: string; code?: string; name?: string; courseId?: string; parts: CoursePartInput[]; notes?: string };
type SemesterInput = { _uid: string; name: string; index: number; type?: COURSE_TYPE; startAt?: string; endAt?: string; notes?: string; courses: CourseAssignmentInput[] };
...
export default function useBatchForm() {
    ...
    /* --------- Default factories ---------- */
    const emptyResultComponent = (): ResultComponentDefInput => ({ _uid: uuidv4(), name: RESULT_COMPONENT_NAME.TT, maxMarks: 100 });
    ...
    const emptyTeacher = (): TeacherInput => ({
        _uid: uuidv4(), name: "", designation: "", notes: ""
    });
    ...
    const emptyCourseAssignment = (): CourseAssignmentInput => ({ _uid: uuidv4(), code: "", name: "", courseId: "", parts: [emptyCoursePart()], notes: "" });
    ...
    /* --------- Payload builders ---------- */
    function buildCreatePayload(values: BatchNestedFormValues): CreateBatchPayload {
        console.log("buildCreatePayload start", values);
        try {
            const payload: CreateBatchPayload = { name: String(values.name ?? "").trim() };

            if (values.program && String(values.program).trim() !== "") payload.program = String(values.program).trim();
            if (values.year && String(values.year).trim() !== "") payload.year = Number(String(values.year).trim());
    ...
    function buildUpdatePayload(values: BatchNestedFormValues, id: string): UpdateBatchPayload {
